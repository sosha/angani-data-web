...
$valid = ['home','search','catalogue','module','detail','pricing','login','register','forgot','reset','dashboard','answer','account','admin','contact','terms','privacy','data-quality','beta-status'];
...
<header class="site-header"><div class="header-bar"><a class="brand-lockup" href="?page=home"><img src="assets/angani-logo-white.png" alt="Angani" class="brand-logo"><span><strong>Angani Data</strong><small>Aviation Intelligence Atlas <sup style="background:#d4a843;color:#071522;font-size:10px;padding:1px 6px;border-radius:4px;font-weight:700;letter-spacing:.5px">BETA</sup></small></span></a><div class="header-actions"><button class="menu-toggle" id="menuToggle" aria-expanded="false" aria-controls="mainNav"><i class="fas fa-bars"></i> Menu</button><button class="search-toggle" id="searchToggle" aria-label="Search" aria-expanded="false"><i class="fas fa-search"></i></button><?php if($user): ?><a class="profile-icon" href="?page=account" aria-label="Profile"><i class="far fa-address-card"></i></a><?php endif; ?></div></div><nav class="main-nav" id="mainNav"><a class="<?=active_page($page,'home')?>" href="?page=home"><i class="fas fa-home"></i>Overview</a><a href="?page=countries"><i class="fas fa-globe"></i>Countries</a><a href="?page=airlines"><i class="fas fa-plane-departure"></i>Airlines</a><a href="?page=airports"><i class="fas fa-map-location"></i>Airports</a><a href="?page=aircraft_types"><i class="fas fa-plane"></i>Aircraft Types</a><a class="<?=active_page($page,'catalogue')?>" href="?page=catalogue"><i class="fas fa-database"></i>All Datasets</a><a class="<?=active_page($page,'pricing')?>" href="?page=pricing"><i class="fas fa-credit-card"></i>Pricing</a><?php if($user): ?><a class="<?=active_page($page,'dashboard')?>" href="?page=dashboard"><i class="fas fa-chart-bar"></i>Dashboard</a><?php if(is_admin()): ?><a class="<?=active_page($page,'admin')?>" href="admin/"><i class="fas fa-shield-alt"></i>Admin</a><?php endif; ?><a href="?page=account"><i class="far fa-address-card"></i> Profile</a><?php else: ?><a class="<?=active_page($page,'login')?>" href="?page=login"><i class="fas fa-sign-in-alt"></i>Log in</a><?php endif; ?></nav></header><div class="search-overlay" id="searchOverlay" hidden><form class="search-overlay-form" method="get"><input type="hidden" name="page" value="search"><input class="search-overlay-input" name="q" placeholder="Search airlines, airports, aircraft types, codes..." autocomplete="off"><button class="btn ink">Search</button></form></div>
...
<section class="view home-view"><div class="hero premium-hero"><div><div class="kicker">Angani Solutions · Aviation Data Product</div><h1>Static aviation intelligence arranged for decisions.</h1><p class="lead">A serious PHP/MySQL aviation data platform for countries, airlines, airports, infrastructure, aircraft types, lessors, commercial reference, IATA/IOSA, regulatory and route intelligence.</p><div class="hero-actions"><a class="btn primary" href="?page=register">Create account</a><a class="btn secondary" href="?page=catalogue">Browse databases</a><a class="btn secondary" href="?page=pricing">View access</a></div></div><aside class="hero-panel cockpit-panel"><div class="hero-slider" id="heroSlider"><?php $slides=rows('SELECT * FROM site_slides WHERE is_active=1 ORDER BY display_order ASC'); if(!$slides){ $slides=[['slide_key'=>'default','title'=>'Data product','subtitle'=>'Admin CRUD, import/export, user tiers, drill-down pages, reference codes and source/change controls are built into the platform.','image_url'=>'','stat_label'=>'Active Airlines','stat_value'=>nfmt($stats['active_airlines']),'link_url'=>'?page=catalogue']]; } foreach($slides as $i=>$s): ?><div class="hero-slide<?=$i===0?' active':''?>" data-index="<?=$i?>"><div class="hero-slide-img" style="background-image:url('<?=e($s['image_url'])?>')"></div><div class="hero-slide-body"><div class="cockpit-line"><span><?=e($s['stat_label']??'')?></span><?php if(isset($s['stat_value'])): ?><b class="active-pill"><?=e($s['stat_value'])?></b><?php endif; ?></div><h2><?=e($s['title'])?></h2><p><?=e($s['subtitle']??'')?></p><?php if($s['link_url']??''): ?><a class="btn ghost" href="<?=e($s['link_url'])?>">Explore →</a><?php endif; ?></div></div><?php endforeach; ?></div><div class="hero-slider-dots"><?php foreach($slides as $i=>$s): ?><span class="hero-slider-dot<?=$i===0?' active':''?>" data-index="<?=$i?>"></span><?php endforeach; ?></div></aside></div>
...
<section class="view"><?php if(!$cfg): ?><div class="empty-state"><h2>Unknown database</h2></div><?php elseif(!module_allowed($cfg)): ?><?=access_gate($cfg['label'].' is a Pro dataset','Create or upgrade your account to access this database.','View pricing')?><?php else: $expandable = in_array($cfg['card']??'', ['country','airline','airport','aircraft','aircraft_type'], true); if($expandable){ $ob=in_array($key,['countries','airlines'],true)?'RAND()':null; [$records,$total]=query_module_records($cfg,24,(page_num()-1)*24,false,$ob); $fo=[]; $extra=''; if($key==='airlines'){ $cv=getv('q'); $cs=getv('status'); $cc=getv('country'); $extra='<form method="get" class="xcard-toolbar"><input type="hidden" name="page" value="airlines"><div class="xcard-search-wrap"><svg style="position:absolute;left:12px;top:50%;transform:translateY(-50%);color:var(--muted);pointer-events:none" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg><input type="text" class="xcard-search" name="q" value="'.e($cv).'" placeholder="Search airlines..." autocomplete="off"></div><select name="status" class="xcard-search xcard-jump" onchange="this.form.submit()"><option value="">All statuses</option><option value="active"'.($cs==='active'?' selected':'').'>Active</option><option value="defunct"'.($cs==='defunct'?' selected':'').'>Defunct</option></select>'.country_select('country',$cc).'<button class="btn ink">Filter</button><a class="btn ghost" href="?page=airlines">Reset</a></form>'; try{ $alRows=rows('SELECT icao_code,iata_code,name FROM airlines WHERE icao_code IS NOT NULL AND icao_code!="" ORDER BY name ASC'); $extra.='<div class="xcard-search-wrap xcard-jump-wrap"><select class="xcard-search xcard-jump" onchange="if(this.value)location.href=\'?page=detail&amp;module=airlines&amp;id=\'+this.value"><option value="">Jump to airline...</option>'; foreach($alRows as $a){ $codes=trim($a['iata_code'].' / '.$a['icao_code'],' /'); $label=e($a['name']).($codes?' ('.$codes.')':''); $extra.='<option value="'.e($a['icao_code']).'">'.$label.'</option>'; } $extra.='</select></div>'; }catch(Throwable $e){} } elseif($key==='countries') $fo=['africa'=>'Africa','asia'=>'Asia','europe'=>'Europe','north america'=>'N America','south america'=>'S America','oceania'=>'Oceania']; elseif($key==='airports') $fo=['large_airport'=>'Large','medium_airport'=>'Medium','small_airport'=>'Small','heliport'=>'Heliport','closed'=>'Closed','seaplane_base'=>'Seaplane']; echo render_xcard_page($key,$cfg,$records,$total,$fo,$extra); echo paginate($total,24); } else { [$records,$total]=query_module_records($cfg,24,(page_num()-1)*24); ?><section class="section-head"><div><div class="eyebrow"><?=e($cfg['tier']==='free'?'Open database':'Pro database')?></div><h1><?=e($cfg['label'])?></h1></div><p><?=nfmt($total)?> records. Admin can add/edit/import; logged-in Pro users can export filtered data.</p></section><?=render_search_bar($key,$cfg)?><?php if(in_array($cfg['card']??'', ['airline','airport','aircraft_type','lessor','country','aircraft'], true)): ?><?=render_module_cards($key,$records)?><?php else: ?><?=render_table($records,$cfg['list'],$key)?><?php endif; ?><?=paginate($total,24)?><?php } endif; ?></section>
...
if($key==='countries'){
    ...
    $mfrLogo=manufacturer_logo_url($r['manufacturer']??'');
    $imgHtml=$mfrLogo?'<img class="hero-logo ac-hero-img" src="'.$mfrLogo.'" alt="'.e($r['manufacturer']??'').'" style="width:auto;height:60px;max-width:140px;border-radius:8px;object-fit:contain;background:rgba(246,241,231,.95);padding:4px 12px">':($acImage?'<img class="hero-logo ac-hero-img" src="'.$acImage.'" alt="'.e($r[$cfg['title']]??'').'" style="width:auto;height:120px;max-width:100%;border-radius:16px">':'<div class="ac-placeholder" style="width:120px;height:120px;background:rgba(7,21,33,.06);border-radius:16px;display:flex;align-items:center;justify-content:center;font-size:3rem;font-weight:700;color:var(--muted);border:1px solid var(--line)">'.e($acIcao?:'✈').'</div>');
...
if(is_logged_in()){ $myV=row('SELECT * FROM record_verifications WHERE entity_type=? AND entity_id=? AND user_id=?',[$key,(int)$id,(int)current_user()['id']]); $myCompl=$myV['completeness']??''; $myAcc=$myV['accuracy']??''; $vForm='<form method="post" class="verify-form" style="display:inline-flex;gap:6px;align-items:center;margin-left:8px">'.csrf_field().'<input type="hidden" name="action" value="record_verify"><input type="hidden" name="entity_type" value="'.e($key).'"><input type="hidden" name="entity_id" value="'.$id.'"><span style="font-size:12px;color:var(--muted)">Mark:</span><select name="completeness" onchange="this.form.submit()"><option value="">Complete?</option><option value="complete"'.($myCompl==='complete'?' selected':'').'>Complete</option><option value="incomplete"'.($myCompl==='incomplete'?' selected':'').'>Incomplete</option></select><select name="accuracy" onchange="this.form.submit()"><option value="">Accuracy?</option><option value="accurate"'.($myAcc==='accurate'?' selected':'').'>Accurate</option><option value="inaccurate"'.($myAcc==='inaccurate'?' selected':'').'>Inaccurate</option><option value="unverified"'.($myAcc==='unverified'?' selected':'').'>Unverified</option></select></form>'; echo $vForm; }
echo '</div>';
echo '</section>';
...
<section class="auth-shell"><div class="panel auth-card"><div class="eyebrow">Member access</div><h1>Log in</h1><form method="post" class="stack-form"><?=csrf_field()?><input type="hidden" name="action" value="login"><label>Email<input name="email" type="email" required></label><label>Password<div class="password-wrap"><input name="password" type="password" id="loginPassword" required><button type="button" class="password-toggle" onclick="togglePassword()" aria-label="Show password"><i class="fas fa-eye"></i></button></div></label><button class="btn ink" type="submit">Log in</button></form><p><a class="linkish" href="?page=forgot">Forgot password?</a> &middot; New user? <a class="linkish" href="?page=register">Create account</a></p></div></section>
<script>function togglePassword(){var p=document.getElementById('loginPassword'),b=p.nextElementSibling;if(p.type==='password'){p.type='text';b.innerHTML='<i class="fas fa-eye-slash"></i>';b.setAttribute('aria-label','Hide password')}else{p.type='password';b.innerHTML='<i class="fas fa-eye"></i>';b.setAttribute('aria-label','Show password')}}</script>
...
<?php elseif($page==='forgot'): $resetSent=getv('sent'); ?>
<section class="auth-shell"><div class="panel auth-card"><div class="eyebrow">Reset password</div><h1>Forgot your password?</h1><?php if($resetSent): ?>
<p class="muted">If that email exists, a reset link has been sent. Check your inbox and follow the instructions. The link expires in 60 minutes.</p>
<?php else: ?>
<form method="post" class="stack-form"><?=csrf_field()?><input type="hidden" name="action" value="forgot_password"><label>Email<input name="email" type="email" required placeholder="Enter your account email"></label><button class="btn ink" type="submit">Send reset link</button></form>
<p><a class="linkish" href="?page=login">Back to login</a></p>
<?php endif; ?></div></section>
<?php elseif($page==='reset'): $rt=getv('token'); $re=getv('e'); $error=getv('error'); ?>
<section class="auth-shell"><div class="panel auth-card"><div class="eyebrow">Reset password</div><h1>Choose a new password</h1><?php if($error): ?><div class="flash error"><?=e($error)?></div><?php endif; ?>
<form method="post" class="stack-form"><?=csrf_field()?><input type="hidden" name="action" value="reset_password"><input type="hidden" name="token" value="<?=e($rt)?>"><input type="hidden" name="email" value="<?=e($re)?>"><label>New password<input name="password" type="password" minlength="8" required></label><label>Confirm password<input name="confirm" type="password" minlength="8" required></label><button class="btn ink" type="submit">Reset password</button></form></div></section>
<?php elseif($page==='account'): if(!$user) redirect_to('?page=login'); ?>
<section class="view"><section class="section-head"><div><div class="eyebrow">Account</div><h1><?=e($user['name'])?></h1></div><?=tier_badge($user)?></section><div class="panel-grid"><div class="panel"><h3>Profile</h3><form method="post" class="stack-form"><?=csrf_field()?><input type="hidden" name="action" value="update_account"><label>Name<input name="name" value="<?=e($user['name'])?>"></label><label>Email<input value="<?=e($user['email'])?>" disabled></label><button class="btn ink">Save</button></form></div><div class="panel"><h3>Account details</h3><div class="xcard-row"><span class="xcard-row-k">Created</span><span class="xcard-row-v"><?=e($user['created_at'] ?? '—')?></span></div><div class="xcard-row"><span class="xcard-row-k">Last login</span><span class="xcard-row-v"><?=e($user['last_login_at'] ?? '—')?></span></div><div class="xcard-row"><span class="xcard-row-k">Role</span><span class="xcard-row-v"><?=e(ucfirst($user['role'] ?? 'user'))?></span></div><p style="margin-top:12px"><a class="linkish" href="<?=e(public_url_prefix())?>?page=forgot">Change password</a></p></div><div class="panel"><h3>Access</h3><p>Your current tier is <strong><?=e($user['tier_name'])?></strong>.</p><p>CSV exports and premium drill-downs are available on Pro and Ultimate according to admin-controlled Tier Visibility rules.</p><?php $tierCode=$user['tier_code']??''; if($tierCode==='free'): ?><a class="btn primary" href="?page=register" style="display:inline-flex;align-items:center;gap:6px"><i class="fas fa-arrow-up"></i> Upgrade to Pro</a><?php elseif($tierCode==='pro'): ?><a class="btn primary" href="?page=contact" style="display:inline-flex;align-items:center;gap:6px"><i class="fas fa-arrow-up"></i> Upgrade to Ultimate</a><?php else: ?><a class="btn primary" href="?page=pricing">Compare plans</a><?php endif; ?></div></div></section>
...
<section class="view"><div class="panel contact-panel"><div class="eyebrow">Ultimate package</div><h1>Contact Angani Solutions</h1><p>For bulk exports, API access, private dashboards, custom aviation datasets and advisory work tailored to the client’s aviation workflow.</p><p><strong>Silas Savali</strong><br>Angani Solutions<br><a href="mailto:info@angani.co.uk">info@angani.co.uk</a><br><a href="https://www.angani.co.uk">www.angani.co.uk</a></p></div></section>’s aviation workflow.</p><p><strong>Silas Savali</strong><br>Angani Solutions<br><a href="mailto:info@angani.co.uk">info@angani.co.uk</a><br><a href="https://www.angani.co.uk">www.angani.co.uk</a></p></div></section>
<?php elseif($page==='terms'): $p=row('SELECT * FROM site_pages WHERE page_key=?',['terms']); ?>
<section class="view static-page"><div class="panel"><?php if($p): ?><div class="eyebrow"><?=e($p['title'])?></div><div class="page-content"><?=$p['content']?></div><?php else: ?><div class="empty-state"><h2>Page not found</h2></div><?php endif; ?></div></section>
<?php elseif($page==='privacy'): $p=row('SELECT * FROM site_pages WHERE page_key=?',['privacy']); ?>
<section class="view static-page"><div class="panel"><?php if($p): ?><div class="eyebrow"><?=e($p['title'])?></div><div class="page-content"><?=$p['content']?></div><?php else: ?><div class="empty-state"><h2>Page not found</h2></div><?php endif; ?></div></section>
<?php elseif($page==='beta-status'): $p=row('SELECT * FROM site_pages WHERE page_key=?',['beta-status']); ?>
<section class="view static-page"><div class="panel"><?php if($p): ?><div class="eyebrow"><?=e($p['title'])?></div><div class="page-content"><?=$p['content']?></div><?php else: ?><div class="empty-state"><h2>Page not found</h2></div><?php endif; ?></div></section>
<?php elseif($page==='data-quality'): ?>
<section class="view static-page"><div class="eyebrow">Data Quality Dashboard</div><h1>Dataset verification & completeness</h1><p>Below is the current status of all datasets. Records with all fields populated are marked complete. Users can mark individual records as complete/incomplete and accurate/inaccurate from any detail page.</p>
<div class="dq-grid"><?php foreach(modules() as $k=>$c){ $t=$c['table']; $cnt=module_count($k); if($cnt===0) continue; $fs=$c['fields']??[]; $complete=0; try{$all=rows("SELECT * FROM `$t` LIMIT 1001");}catch(Throwable $e){$all=[];} $total=count($all); if($total>1000){ $complete=-1; }else{ foreach($all as $r){ $allFilled=true; foreach($fs as $f){ if(!isset($r[$f])||$r[$f]===null||$r[$f]===''){$allFilled=false;break;}} if($allFilled)$complete++; } } $verified=admin_safe_count("SELECT COUNT(DISTINCT entity_id) FROM record_verifications WHERE entity_type=? AND completeness='complete'",[$k]); $accurate=admin_safe_count("SELECT COUNT(DISTINCT entity_id) FROM record_verifications WHERE entity_type=? AND accuracy='accurate'",[$k]); $inaccurate=admin_safe_count("SELECT COUNT(DISTINCT entity_id) FROM record_verifications WHERE entity_type=? AND accuracy='inaccurate'",[$k]); ?><article class="dq-card"><div class="dq-head"><h3><?=e($c['label'])?></h3><span class="dq-count"><?=nfmt($cnt)?> records</span></div><div class="dq-stats"><div class="dq-stat"><span class="dq-stat-label">Complete</span><span class="dq-stat-value"><?=$complete===-1?'>1K':nfmt($complete)?></span><span class="dq-stat-pct"><?=$cnt>0&&$complete>=0?round($complete/$cnt*100):($complete===-1?'—':'0').'%'?></span></div><div class="dq-stat"><span class="dq-stat-label">Verified</span><span class="dq-stat-value"><?=nfmt($verified)?></span></div><div class="dq-stat"><span class="dq-stat-label">Accurate</span><span class="dq-stat-value dq-accurate"><?=nfmt($accurate)?></span></div><div class="dq-stat"><span class="dq-stat-label">Inaccurate</span><span class="dq-stat-value dq-inaccurate"><?=nfmt($inaccurate)?></span></div></div><div class="dq-bar"><div class="dq-bar-fill" style="width:<?=$cnt>0&&$complete>=0?$complete/$cnt*100:0?>%"></div></div></article><?php } ?></div></section>
<?php endif; ?></main><footer class="site-footer"><div class="footer-inner"><div class="footer-brand"><strong>Angani Data</strong><span>Aviation Intelligence Atlas <sup style="background:#d4a843;color:#071522;font-size:10px;padding:1px 6px;border-radius:4px;font-weight:700;letter-spacing:.5px">BETA</sup></span><p>Built for aviation professionals, consultants, airports, airlines and regulators.</p></div><div class="footer-links"><a href="?page=terms"><i class="fas fa-file-contract"></i> Terms and Conditions</a><a href="?page=privacy"><i class="fas fa-shield-alt"></i> Privacy Policy</a><a href="?page=data-quality"><i class="fas fa-check-double"></i> Data Quality</a><a href="?page=beta-status"><i class="fas fa-flask"></i> Beta Status</a></div></div><div class="footer-bottom"><span>© <?=date('Y')?> Angani Solutions</span></div></footer><script src="js/app.js"></script></body></html>
