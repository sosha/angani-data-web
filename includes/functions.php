...
function render_record_card(string $key,array $cfg,array $r): string {
    ...
    if($card==='country'){
        ...
        $name=e($title?:$cfg['label']); $cont=strtolower(e($r['continent']??''));
...
function rsh(cd){var b=cd.querySelector(".xcard-expand-btn");if(b)b.textContent="+ Expand";var h=cd.querySelector(".xcard-hint");if(h)h.textContent="Click to expand"}
function cl(){ca.forEach(function(x){x.classList.remove("expanded");rsh(x)});var dp=g.querySelector(".xcard-detail-panel");if(dp)dp.remove()}
ca.forEach(function(cd){cd.addEventListener("click",function(ev){if(ev.target.closest("a"))return;var w=this.classList.contains("expanded");cl();if(!w){this.classList.add("expanded");var b=this.querySelector(".xcard-expand-btn");if(b)b.textContent="- Collapse";var h=this.querySelector(".xcard-hint");if(h)h.textContent="";var ec=this.querySelector(".xcard-expand");if(ec){var dp=document.createElement("div");dp.className="xcard-detail-panel";dp.innerHTML=ec.innerHTML;this.parentNode.insertBefore(dp,this.nextSibling)}this.scrollIntoView({behavior:"smooth",block:"nearest"})}})});
