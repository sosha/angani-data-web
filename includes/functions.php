...
function render_record_card(string $key,array $cfg,array $r): string {
    ...
    if($card==='country'){
        ...
        $region=e($r['un_region']??$r['continent']??''); $capital=e($t['capital']??''); $lang=e($t['official_languages']??'');
...
ca.forEach(function(cd){cd.addEventListener("click",function(ev){if(ev.target.closest(".xcard-detail-btn")||ev.target.closest("a"))return;var w=this.classList.contains("expanded");document.querySelectorAll(".xcard.expanded").forEach(function(x){x.classList.remove("expanded");var b=x.querySelector(".xcard-expand-btn");if(b)b.textContent="+ Expand";var h=x.querySelector(".xcard-hint");if(h)h.textContent="Click to expand"});if(!w){this.classList.add("expanded");var b=this.querySelector(".xcard-expand-btn");if(b)b.textContent="- Collapse";var h=this.querySelector(".xcard-hint");if(h)h.textContent="";this.scrollIntoView({behavior:"smooth",block:"nearest"})}})});
...
f.forEach(function(b){b.addEventListener("click",function(){f.forEach(function(x){x.classList.remove("active")});this.classList.add("active");var ff=this.dataset.f;document.querySelectorAll(".xcard.expanded").forEach(function(x){x.classList.remove("expanded")});ca.forEach(function(cd){cd.style.display=(ff==="all"||cd.dataset.xf===ff)?"":"none"});u()})});
s.addEventListener("input",function(){clearTimeout(t);t=setTimeout(function(){var q=this.value.trim().toLowerCase();document.querySelectorAll(".xcard.expanded").forEach(function(x){x.classList.remove("expanded")});ca.forEach(function(cd){cd.style.display=(cd.dataset.xs&&cd.dataset.xs.indexOf(q)!==-1)?"":"none"});u()}.bind(this),150)});
})();
</script>';
    return $html;
}
