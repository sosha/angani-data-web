...
function cl(){ca.forEach(function(x){x.classList.remove("expanded")});var dp=g.querySelector(".xcard-detail-panel");if(dp)dp.remove()}
ca.forEach(function(cd){cd.addEventListener("click",function(ev){if(ev.target.closest(".xcard-detail-btn")||ev.target.closest("a"))return;var w=this.classList.contains("expanded");cl();if(!w){this.classList.add("expanded");var ec=this.querySelector(".xcard-expand");if(ec){var dp=document.createElement("div");dp.className="xcard-detail-panel";dp.innerHTML=ec.innerHTML;this.parentNode.insertBefore(dp,this.nextSibling)}var h=this.querySelector(".xcard-hint");if(h)h.textContent="";this.scrollIntoView({behavior:"smooth",block:"nearest"})}})});
